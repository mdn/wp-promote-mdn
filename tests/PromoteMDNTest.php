...
class PromoteMDNTest extends PHPUnit_Framework_TestCase {
    ...
    public function test_options_injection() {
        $this->assertEquals(
                'https://developer.mozilla.org/test', $this->pm->options[ 'customkey_url' ]
        );
    }
    ...
    public function test_custom_key() {
        ...
        $this->assertEquals(
                '<p>' . $this->js_linked . ' and <a target="_blank" title="groovecoder" href="http://groovecoder.com' . $this->expected_tracking_querystring . '" class="promote-mdn">groovecoder</a></p>', $this->pm->process_text( $text )
        );
    }
