...
if ( !class_exists( 'PromoteMDN' ) ) :
  ...
  class PromoteMDN {
    ...
    public $install_options = array(
	  ...
	  'maxsingle' => 1,
	  'customkey' => '',
	  'customkey_url' => 'https://raw.githubusercontent.com/mdn/wp-promote-mdn/def-list/terms.txt',
	  'customkey_url_expire' => 86400,
	  'blanko' => 'on',
	  'add_src_param' => 'on',
	  'allowfeed' => '',
	  'maxsingleurl' => '1',
	  'hide_notices' => array( '1.8' => 1 ),
	  'allowcomments' => '',
    );
	...
	if ( is_array( $options[ 'ignoreposttype' ] ) ) {
	  foreach ( $options[ 'ignoreposttype' ] as $posttype => $value ) {
	    $options[ 'ignoreposttype' ][ $posttype ] = $options[ 'ignoreposttype' ][ $posttype ] == 'on' ? 'checked' : '';
	  }
	}
	$ignoreposttype = $options[ 'ignoreposttype' ];
		  ...
		  <div class="dbx-content">
			...
			<form name="PromoteMDN" action="<?php echo esc_html( $action_url ) ?>" method="post">
			    ...
			    <p><?php _e( 'Load keywords from URL:', 'promote-mdn' ) ?>
				  ...
				  <a class="button-secondary" id="preview" href="<?php echo esc_html( $customkey_url ) ?>" target="_blank"><?php _e( 'Preview', 'promote-mdn' ) ?></a><br>
			    ...
			    <pre>addons, amo, http://addons.mozilla.org/</pre>
			    <pre>sumo, http://support.mozilla.org/</pre>
			    </p>
	...
	return array(
	    ...
	    '1.8' => sprintf( __( 'Now the terms list is on GitHub to improve the term list easily!', 'promote-mdn' ), 'widgets.php' ),
	);
    }
	...
	$last_version = key( array_reverse( $this->get_version_notices() ) );
	if ( !isset( $hide_notices[ $last_version ] ) ) {
	  foreach ( $this->get_version_notices() as $version => $notice ) {
		...
		?>
		<div class="updated notice" data-href="<?php echo esc_html( $this->hide_href( $version ) ) ?>"><p class="promote-mdn-notice"><a href="options-general.php?page=promote-mdn.php"><?php _e( 'Promote MDN', 'promote-mdn' ) ?></a> <?php echo esc_html( $version ) ?> - <?php echo $notice ?></p></div>
		<?php
	    }
	  }
	}
    ...
    function upgrade_18() {
	$options = get_option( $this->option_name );
	if ( $options[ 'customkey_url' ] === 'https://developer.mozilla.org/en-US/docs/Template:Promote-MDN?raw=1' ) {
	  $options[ 'customkey_url' ] = 'https://raw.githubusercontent.com/mdn/wp-promote-mdn/def-list/terms.txt';
	  update_option( $this->option_name, $options );
	  $this->reload_value( $options[ 'customkey_url' ] );
	}
    }
